...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        float length=0,distance=10;
        int j=0,wordlenght=0;
        for (int i = 0; i < arr.getSize(); i++) {
            length+=arr.get(i).length();
        }
        length=length/arr.getSize();
        for (int i = 0; i < arr.getSize(); i++) {
            if(Math.abs(arr.get(i).length()-length)<=distance){
                if(arr.get(i).length()>wordlenght){
                    distance=Math.abs(arr.get(i).length()-length);
                    j=i;
                    wordlenght=arr.get(i).length();
                }
            }
        }
        return arr.get(j);

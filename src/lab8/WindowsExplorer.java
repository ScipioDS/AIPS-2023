...
//    }
    public Node<E> addChild(Node<E> node, E elem) {
        SLLNode<E> tmp = new SLLNode<E>(elem);
        SLLNode<E> curr = (SLLNode<E>) node;
        SLLNode<E> prev = curr.firstChild;
        tmp.parent = curr;
//        tmp.sibling = curr.firstChild;
//        curr.firstChild = tmp;
//        tmp.parent = curr;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (j = 0; j < commands.length; j++) {
            ...
            if (parts[0].equals("CREATE")){
                tree.addChild(tmp,parts[1]);
            } else if (parts[0].equals("OPEN")) {
                if (tree.childCount(tmp)==0) continue;
                SLLNode<String> child = tmp.firstChild;
                    for (k = 0; k < tree.childCount(tmp); k++) {
                        if (child.element.equals(parts[1])){
                            tmp = child;
                            break;
                        }
                        child = child.sibling;
                ...
                Stack<String> stack = new Stack<>();
                    SLLNode<String> tmpit = tmp;
                    while (tmpit!=null){
                        stack.push("\\");
                        stack.push(tmpit.element);
                        tmpit = tmpit.parent;
                    }
                    String output = "";
                    while (!stack.isEmpty()){
